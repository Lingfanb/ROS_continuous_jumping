...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world = coordinateRotation(CoordinateAxis::X, seResult.rpy[0]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() *  v_des_robot;
 ...
 //Integral-esque pitch and roll compensation
  ...
  if(iterationCounter % (iterationsBetweenMPC * horizonLength) == 0){
    for(int i = 0; i < 4; i++)
      ...
      a = (((W.transpose() * W).transpose() * (W.transpose() * W)).inverse() * (W.transpose() * W).transpose()) * W.transpose() * pz;
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_yaw = coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])*coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) 
                        *coordinateRotation(CoordinateAxis::X, seResult.rpy[0]) * seResult.vWorld;
    ...
    double Q[12] = {1.5, 1.5, 10, 1.5, 1.5, 20, 0, 0, 0.6, 0.5, 0.5, 0.2};
    ...
    Vec3<double> v_des_robot(stateCommand->data.stateDes[6], stateCommand->data.stateDes[7],0);
    Vec3<double> v_des_world = coordinateRotation(CoordinateAxis::X, seResult.rpy[0]).transpose() *
    coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() *coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() * v_des_robot;
    ...
    // else
     //{
      ...
      world_position_desired[2] = a[0] + a[1] * xStart + a[2] * yStart + 0.3;
      
      
      //printf("zys: \t%.3f\t%3.f\n", world_position_desired[2], yStart);
      //printf("perr \t%.3f\t%.3f\n", p[0] - world_position_desired[0], p[1] - world_position_desired[1]);

      double trajInitial[12] = {rpy_int[0],  // 0
                                rpy_int[1],    // 1
                                world_position_desired[2], //stateCommand->data.stateDes[5],    // 2
                                xStart,                                   // 3
                                yStart,                                   // 4
    ...
    t1.start();
    dtMPC = dt * iterationsBetweenMPC;
    setup_problem(dtMPC,horizonLength,0.6,500);
