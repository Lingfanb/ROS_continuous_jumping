...
//  debugSphere->radius = 1;
 ...
 //Integral-esque pitch and roll compensation
  if(fabs(v_robot[0]) > 1)   //avoid dividing by zero
  ...
  if(fabs(v_robot[1]) > 0.2)
  {
    rpy_int[0] += dt*(stateCommand->data.stateDes[3] -ground_pitch
    ...
    // else
     //{
      ...
      double trajInitial[12] = { 0, //rpy_comp[0],  // 0
                                rpy_comp[1] + ground_pitch + stateCommand->data.stateDes[4],    // 1
