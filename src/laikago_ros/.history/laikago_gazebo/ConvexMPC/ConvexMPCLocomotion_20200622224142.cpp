...
ConvexMPCLocomotion::ConvexMPCLocomotion(double _dt, int _iterations_between_mpc) :
  ...
  bounding(horizonLength, Vec4<int>(3,3,0,0),Vec4<int>(5,5,5,5),"Bounding"),
...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world =  (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])*coordinateRotation(CoordinateAxis::X, seResult.rpy[0])).transpose() *  v_des_robot;
  Vec3<double> v_robot = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])) *seResult.vWorld;
 ...
 // printf("p_des \t%.6f\n", dt * v_des_world[0])
 //stateCommand->data.stateDes[4] += ground_slope;
 //Integral-esque pitch and roll compensation
  if(fabs(v_robot[0]) > 0.2)   //avoid dividing by zero
  {
    rpy_int[1] += dt*(ground_pitch- seResult.rpy[1])/v_robot[0];
  }
  //std::cout << "rpy _ int:  " << rpy_int[1] << std::endl;
  ...
  //std::cout << "pitch des" << rpy_comp[1] << std::endl;
  ...
  for(int i = 0; i < 4; i++) {
   ...
   // pFoot[i] = data._legController->data[i].p;
    //std::cout << "pFoot" << i << "\n" << pFoot[i] << std::endl;
   pFoot_yaw[i] = coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() * (data._quadruped->getHipLocation(i) + data._legController->data[i].p);
  }
  ...
  // get ground pitch each gait cycle
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(firstSwing[i]) {
      if(iterationCounter %  (horizonLength * iterationsBetweenMPC)== 0){
         W << 1, pFoot_yaw[0][0], pFoot_yaw[0][1],
              0, pFoot_yaw[1][0], pFoot_yaw[1][1],
              0, pFoot_yaw[2][0], pFoot_yaw[2][1],
              0, pFoot_yaw[3][0], pFoot_yaw[3][1];
    for(int i = 0; i < 4; i++)
        {
      ...
      a =  (W.transpose()* W *W.transpose()*W).inverse()* W.transpose() * W * W.transpose() * pz;
      ground_pitch = acos(-1/sqrt(a[1]*a[1] + a[2]*a[2] +1)) - 3.14159;
      
      std::cout << "ground pitch: " << ground_pitch << std::endl;
  }
      
      footSwingTrajectories[i].setHeight(.1);
      Vec3<double> offset(0, side_sign[i] * .083, 0); // y_offset for AlienGo 0.083; A1: 0.085
      ...
      double pfx_rel = seResult.vWorld[0] * .5 * gait->_stance * dtMPC +
                      .02*(seResult.vWorld[0]-v_des_world[0]) +
      ...
      double pfy_rel = seResult.vWorld[1] * .5 * gait->_stance * dtMPC +
                      .02*(seResult.vWorld[1]-v_des_world[1]) +
      ...
      //Pf[2] = -0.01;
      Pf[2] = a[0] + a[1] * Pf[0] + a[2] * Pf[1];// pFoot[i][2];
      //Pf[2] = 0;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_yaw = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) *coordinateRotation(CoordinateAxis::X, seResult.rpy[0])).transpose()
                        * seResult.vWorld;
    ...
    double Q[12] = {0.25, 0.25, 10, 2.5, 2.5, 20, 0, 0, 0.4, 0.3, 0.3, 0.3};
    ...
    Vec3<double> v_des_robot(stateCommand->data.stateDes[6], stateCommand->data.stateDes[7],0);
    Vec3<double> v_des_world = 
   (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])) * v_des_robot;
    ...
    // else
     //{
      ...
      world_position_desired[2] = 0.3 + (a[0] + a[1] * xStart + a[2] * yStart);
      
     // std::cout << "yaw des ： " << stateCommand->data.stateDes[5] << std::endl; 
      //printf("zys: \t%.3f\t%3.f\n", world_position_desired[2], yStart);
      //printf("perr \t%.3f\t%.3f\n", p[0] - world_position_desired[0], p[1] - world_position_desired[1]);

      double trajInitial[12] = {rpy_comp[0],  // 0
                                rpy_int[1], //rpy_comp[1],    // 1
                                stateCommand->data.stateDes[5],    // 2
                                xStart,                                   // 3
                                yStart,                                   // 4
                                world_position_desired[2],      // 5; for Aliengo robot, change to 0.4
                                0,                                        // 6
                                0,                                        // 7
       ...
       for(int i = 0; i < horizonLength; i++)
      {
        ...
        else
        {
          ...
          //trajAll[12*i + 5] = 0.3 + (a[0] + a[1] * trajAll[12*i + 3] + a[2] * trajAll[12*i +4]); 
    ...
    t1.start();
    dtMPC = dt * iterationsBetweenMPC;
    setup_problem(dtMPC,horizonLength,0.2,500);
    ...
    for(int leg = 0; leg < 4; leg++)
    {
     ...
     // f_ff[leg] =  - coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * f;
      f_ff[leg] = - (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])*coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])* coordinateRotation(CoordinateAxis::X, seResult.rpy[0])).transpose()* f;
