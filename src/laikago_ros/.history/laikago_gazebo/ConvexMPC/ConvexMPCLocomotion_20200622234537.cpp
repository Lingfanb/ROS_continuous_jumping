...
//  debugSphere->radius = 1;
 ...
 // v_des_world = seResult.rBody.transpose() * v_des_robot;
  v_des_world =  (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])) *  v_des_robot;
  Vec3<double> v_robot = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])).transpose() *seResult.vWorld;
 ...
 //Integral-esque pitch and roll compensation
  ...
  {
    rpy_int[1] += dt*(ground_pitch- seResult.rpy[1])/5;
  }
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(firstSwing[i]) {
      ...
      //Pf[2] = -0.01;
      Pf[2] += (a[0] + a[1] * Pf[0] + a[2] * Pf[1] - pFoot[i][2])/5;// pFoot[i][2];
      //Pf[2] = 0;
  ...
  seResult.rBody * v_des_robot;
    ...
    // else
     //{
      ...
      world_position_desired[2] = 0.3 + (a[0] + a[1] * xStart + a[2] * yStart)/10;
     ...
     // std::cout << "yaw des ： " << stateCommand->data.stateDes[5] << std::endl; 
       ...
       for(int i = 0; i < horizonLength; i++)
      {
        ...
        else
        {
          ...
          trajAll[12*i + 5] = 0.3 + (a[0] + a[1] * trajAll[12*i + 3] + a[2] * trajAll[12*i +4])/10; 
