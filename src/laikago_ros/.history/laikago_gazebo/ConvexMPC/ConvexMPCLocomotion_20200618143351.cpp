...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  std::cout << "ground pitch" << ground_slope << std::endl;
 // printf("p_des \t%.6f\n", dt * v_des_world[0])
 ...
 //Integral-esque pitch and roll compensation
  ...
  {
    rpy_int[0] += dt*(stateCommand->data.stateDes[3] - ground_slope /*-hw_i->state_estimator->se_ground_pitch*/
        - seResult.rpy[0])/v_robot[1];
  }

  rpy_int[0] = fminf(fmaxf(rpy_int[0], -.25), .25);
  rpy_int[1] = fminf(fmaxf(rpy_int[1], -.25), .25);
  rpy_comp[1] = v_robot[0] * rpy_int[1] + ground_slope;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_yaw = coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])*coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * seResult.vWorld;
    ...
    double Q[12] = {0.25, 20, 10, 1.5, 1.5, 30, 0, 0, 0.3, 0.2, 0.2, 0.2};
