...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_yaw = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])).transpose()
                        * seResult.vWorld;
  ...
  seResult.rBody * v_des_robot;
    ...
    //f_ff[leg] = -seResult.rBody.transpose() * f;
     // f_ff[leg] =  - coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * f;
      f_ff[leg] = - (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])*coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])).transpose() * f;
