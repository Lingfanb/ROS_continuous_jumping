...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world =  coordinateRotation(CoordinateAxis::X, seResult.rpy[0]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose()
               * v_des_robot;
  Vec3<double> v_robot = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0]))
                *seResult.vWorld;
 ...
 //Integral-esque pitch and roll compensation
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(firstSwing[i]) {
      ...
      Vec3<double> offset(0, side_sign[i] * .085, 0); // y_offset for AlienGo 0.083; A1: 0.085
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_yaw = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0]))
                        * seResult.vWorld;
    ...
    double Q[12] = {0.25, 0.25, 10, 1.5, 1.5, 10, 0, 0, 0.3, 0.2, 0.2, 0.2};
    ...
    Vec3<double> v_des_robot(stateCommand->data.stateDes[6], stateCommand->data.stateDes[7],0);
    Vec3<double> v_des_world = coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose()* v_des_robot;
   // Vec3<double> v_des_world = coordinateRotation(CoordinateAxis::X, seResult.rpy[0]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose()
                 //v_des_robot;
    ...
    // else
     ...
     // std::cout << "yaw des ： " << stateCommand->data.stateDes[5] << std::endl; 
      ...
      double trajInitial[12] = {0, //rpy_comp[0], // 0
                                0, //rpy_comp[1], //rpy_comp[1],    // 1
       ...
       for(int i = 0; i < horizonLength; i++)
      {
        ...
        else
        {
          ...
          //trajAll[12*i + 5] = 0.3 + (a[0] + a[1] * trajAll[12*i + 3] + a[2] * trajAll[12*i +4])/1.2; 
    ...
    t1.start();
    dtMPC = dt * iterationsBetweenMPC;
    setup_problem(dtMPC,horizonLength,0.2,300);
    ...
    f_ff[leg] = -(coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])) * f;
     // f_ff[leg] =  - coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * f;
      //f_ff[leg] = - coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])*coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * f;
