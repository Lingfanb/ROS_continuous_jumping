 ...
 //Integral-esque pitch and roll compensation
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(firstSwing[i]) {
      ...
      //Pf[2] = -0.01;
      Pf[2] = pFoot[i][2] + (a[0] + a[1] * pFoot[i][0] + a[2] * pFoot[i][1] - pFoot[i][2])/2;// pFoot[i][2];
      //Pf[2] = 0;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_des_robot(stateCommand->data.stateDes[6], stateCommand->data.stateDes[7],0);
    Vec3<double> v_des_world = 
   (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])).transpose() * v_des_robot;
    ...
    // else
     //{
      ...
      world_position_desired[2] = 0.3 + (0.3 + a[0] + a[1] * xStart + a[2] * yStart - seResult.position(2))/2;
