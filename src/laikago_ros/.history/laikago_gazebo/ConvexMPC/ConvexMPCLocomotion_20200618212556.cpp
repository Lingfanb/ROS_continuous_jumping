...
//  debugSphere->radius = 1;
 ...
 //Integral-esque pitch and roll compensation
  ...
  //std::cout << "ground pitch" << acos(-1/sqrt(a[1]*a[1] + a[2]*a[2] +1)) - 3.14 << std::endl;
  {
    rpy_int[1] += dt*(stateCommand->data.stateDes[4] -ground_slope -seResult.rpy[1])/v_robot[0];
  }
  if(fabs(v_robot[1]) > 0.1)
  {
    rpy_int[0] += dt*(stateCommand->data.stateDes[3] -ground_slope
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(firstSwing[i]) {
      ...
      ground_slope = acos(-1/sqrt(a[1]*a[1] + a[2]*a[2] +1)) - 3.14;
      footSwingTrajectories[i].setHeight(.1);
      Vec3<double> offset(0, side_sign[i] * .085, 0); // y_offset for AlienGo 0.085; A1: 0.085
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    double Q[12] = {0.25, 0.25, 10, 1.5, 1.5, 30, 0, 0, 0.3, 0.2, 0.2, 0.2};
    ...
    // else
     //{
      ...
      double trajInitial[12] = { rpy_comp[0], //rpy_comp[0],  // 0
                                ground_slope + stateCommand->data.stateDes[4],    // 1
