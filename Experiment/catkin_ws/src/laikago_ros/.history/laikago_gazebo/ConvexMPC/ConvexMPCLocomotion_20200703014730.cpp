...
double Gait::getFlightTime(){
  int flightPhase = _nMPC_segments/2 - _durations[0];
  if(flightPhase <= 0){
    flightPhase = 0;
  } 
  flightTime = flightPhase/(double) _nMPC_segments;
  return flightTime;
}
...
ConvexMPCLocomotion::ConvexMPCLocomotion(double _dt, int _iterations_between_mpc) :
  ...
  horizonLength(10),
  dt(_dt),
  galloping(horizonLength, Vec4<int>(0,2,7,9),Vec4<int>(1,1,1,1),"Galloping"),
  ...
  bounding_op(horizonLength, Vec4<int>(5,5,0,0), Vec4<int>(4,4,4,4))
{
  gaitNumber = 2;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    double Q[12] = {0.25, 0.25, 20, 2.5, 2.5, 30, 0, 0, 0.3, 0.2, 0.2, 0.2};
    ...
    // else
     //{
      ...
      //std::cout << "yaw act: " << seResult.rpy[2] << std::endl;
      //printf("xys: \t%.3f\t%3.f\n", xStart, yStart);
