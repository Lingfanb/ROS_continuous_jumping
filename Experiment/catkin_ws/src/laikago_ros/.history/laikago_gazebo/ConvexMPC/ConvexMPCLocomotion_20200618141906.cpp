...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world = coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * v_des_robot;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_des_robot(stateCommand->data.stateDes[6], stateCommand->data.stateDes[7],0);
    Vec3<double> v_des_world = coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() *  coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * v_des_robot;
