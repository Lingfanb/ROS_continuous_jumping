...
//  debugSphere->radius = 1;
 ...
 //Integral-esque pitch and roll compensation
  ...
  {
    rpy_int[1] += dt*(stateCommand->data.stateDes[3] - seResult.rpy[1])/v_robot[0];
  }
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    for(int i = 0; i < 4; i++)
     ...
     // std::cout << "ground pitch: " << ground_pitch << std::endl;
      ...
      //Pf[2] = -0.01;
      Pf[2] = a[0] + a[1] * pFoot[i][0] + a[2] * pFoot[i][1];// pFoot[i][2];
      //Pf[2] = 0;
   ...
   (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])) * v_des_robot;
    ...
    // else
     //{
      ...
      world_position_desired[2] += (0.3 + a[0] + a[1] * xStart + a[2] * yStart - seResult.position[2])/100;
     ...
     // std::cout << "yaw des ： " << stateCommand->data.stateDes[5] << std::endl; 
       ...
       for(int i = 0; i < horizonLength; i++)
      {
        ...
        else
        {
          ...
          trajAll[12*i + 5] += (a[0] + a[1] * trajAll[12*i + 3] + a[1] * trajAll[12*i +4])/30; 
          //std::cout << "yaw traj" <<  trajAll[12*i + 2] << std::endl;
        }
      }

      std::cout << "z_des: " <<  world_position_desired[2] << std::endl;
    //}
