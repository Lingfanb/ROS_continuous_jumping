...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world =  (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1])*coordinateRotation(CoordinateAxis::X, seResult.rpy[0])) *  v_des_robot;
  Vec3<double> v_robot = (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])).transpose() *seResult.vWorld;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_des_robot(stateCommand->data.stateDes[6], stateCommand->data.stateDes[7],0);
    Vec3<double> v_des_world = 
   (coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])).transpose() * v_des_robot;
    ...
    // else
     //{
      ...
      world_position_desired[2] += (a[0] + a[1] * xStart + a[2] * yStart)/100;
     ...
     // std::cout << "yaw des ： " << stateCommand->data.stateDes[5] << std::endl; 
       ...
       for(int i = 0; i < horizonLength; i++)
      {
        ...
        else
        {
          ...
          trajAll[12*i + 5] += (a[0] + a[1] * trajAll[12*i + 3] + a[2] * trajAll[12*i +4])/100; 
