...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world =  coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])
               * v_des_robot;
  Vec3<double> v_robot = coordinateRotation(CoordinateAxis::X, seResult.rpy[0]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) 
                *seResult.vWorld;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    Vec3<double> v_yaw = coordinateRotation(CoordinateAxis::X, seResult.rpy[0]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]) 
                        * seResult.vWorld;
    ...
    Vec3<double> v_des_world = coordinateRotation(CoordinateAxis::Z, seResult.rpy[2]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::X, seResult.rpy[0])
               * v_des_robot;
