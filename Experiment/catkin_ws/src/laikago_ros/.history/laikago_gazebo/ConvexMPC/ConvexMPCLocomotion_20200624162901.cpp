...
//  debugSphere->radius = 1;
 ...
 // data._stateEstimator->run();
  ...
  v_des_world =  coordinateRotation(CoordinateAxis::X, seResult.rpy[0]).transpose() * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]).transpose() * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])
               * v_des_robot;
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData& data, bool omniMode) 
{
  ...
  if((iterationCounter % iterationsBetweenMPC) == 0)
  {
   // std::cout << "runMPC" << std::endl;
    ...
    f_ff[leg] = -coordinateRotation(CoordinateAxis::X, seResult.rpy[0]) * coordinateRotation(CoordinateAxis::Y, seResult.rpy[1]) * coordinateRotation(CoordinateAxis::Z, seResult.rpy[2])  * f;
