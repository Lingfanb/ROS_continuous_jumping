...
void QPLocomotion::run(ControlFSMData& data)
{
  ...
  for(int foot = 0; foot < 4; foot++)
  {
    ...
    data._legController->commands[foot].pDes = pDesLeg;
    data._legController->commands[foot].vDes = vDesLeg;
    data._legController->commands[foot].feedforwardForce = seResult.rBody.transpose() * f_ff[foot];
...
void QPLocomotion::updateQP(ControlFSMData& data){
  ...
  double Base_weights[3] = {500, 80, 50};
  ...
  for(int i = 0; i < 3; i++){
    ...
    kdCOM[i] = 30;     //_data->controlParameters->kdCOM(i);
